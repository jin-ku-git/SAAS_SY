 ...
 * 商品列表适配器
 */
public class GoodsListAdapter extends RecyclerView.Adapter<GoodsListAdapter.myViewHodler> {
    private Context context;
    private ArrayList<SaleBillBean.GoodsListBean> rechargeBeans;
    private int currentIndex = 0;

    //创建构造函数
    public GoodsListAdapter(Context context, ArrayList<SaleBillBean.GoodsListBean> goodsEntityList) {
        //将传递过来的数据，赋值给本地变量
    ...
    public myViewHodler onCreateViewHolder(ViewGroup parent, int viewType) {
        //创建自定义布局
        View itemView = View.inflate(context, R.layout.item_goods_list_layout, null);
