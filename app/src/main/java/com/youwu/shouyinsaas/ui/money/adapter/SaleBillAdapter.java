...
public class SaleBillAdapter extends RecyclerView.Adapter<SaleBillAdapter.myViewHodler> {
    ...
    public void onBindViewHolder(final myViewHodler holder, @SuppressLint("RecyclerView") final int position) {
        ...
        holder.order_time.setText("订单时间："+data.getCreated_at());//订单时间
        holder.order_prick.setText(data.getPay_amount()+"");//订单金额
        holder.order_type.setText("订单状态："+data.getOrder_status_name());
        if (data.getOrder_status()!=null){
            switch (subZeroAndDot(data.getOrder_status())){
                case "0"://待支付
                    holder.order_type.setTextColor(ContextCompat.getColor(context, R.color.main_yellow));
                    break;
                case "1"://已支付
                    holder.order_type.setTextColor(ContextCompat.getColor(context, R.color.main_green));
                    break;
                case "2"://已取消
                    holder.order_type.setTextColor(ContextCompat.getColor(context, R.color.textColorHint));
                    break;
                case "3"://已退款
                    holder.order_type.setTextColor(ContextCompat.getColor(context, R.color.main_green));
                    break;
    ...
    class myViewHodler extends RecyclerView.ViewHolder {
        ...
        private TextView order_type;//订单状态
        private View mView;
