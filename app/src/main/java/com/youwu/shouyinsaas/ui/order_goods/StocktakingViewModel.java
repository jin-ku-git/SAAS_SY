...
public class StocktakingViewModel extends BaseViewModel<DemoRepository> {
    ...
    //原因列表
    public SingleLiveEvent<ArrayList<ReasonBean>> reasonList = new SingleLiveEvent<>();

    public StocktakingViewModel(@NonNull Application application, DemoRepository repository) {
        ...
        model.GET_REASON()
                ...
                .subscribe(new DisposableObserver<BaseBean<Object>>() {
                    ...
                    public void onNext(BaseBean<Object> response) {
                        if (response.isOk()){
                            String submitJsonData = new Gson().toJson(response.data);
                            ArrayList<ReasonBean> list=  AppApplication.getObjectList(submitJsonData,ReasonBean.class);
    ...
    public void sorting_inventory(String goods_list) {
        model.SORTING_INVENTORY(goods_list)
                .compose(RxUtils.schedulersTransformer()) //线程调度
                .compose(RxUtils.exceptionTransformer())
                .doOnSubscribe(new Consumer<Disposable>() {
                    @Override
