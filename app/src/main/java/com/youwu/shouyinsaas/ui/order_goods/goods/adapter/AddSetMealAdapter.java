...
public class AddSetMealAdapter extends RecyclerView.Adapter<AddSetMealAdapter.myViewHodler> {
    private Context context;
    private List<CommunityBean> goodsEntityList;
    private int currentIndex = 0;

    //创建构造函数
    public AddSetMealAdapter(Context context, List<CommunityBean> goodsEntityList) {
        //将传递过来的数据，赋值给本地变量
    ...
    public myViewHodler onCreateViewHolder(ViewGroup parent, int viewType) {
        //创建自定义布局
        View itemView = View.inflate(context, R.layout.item_add_set_meal_layout, null);
...
//        holder.mItemGoodsImg;
        holder.item_commodity_name.setText(data.getGoods_name());//获取实体类中的name字段并设置
        holder.com_price.setText(data.getGoods_price());//获取实体类中的name字段并设置
