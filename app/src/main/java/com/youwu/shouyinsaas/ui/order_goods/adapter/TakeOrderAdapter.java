 ...
 * 取单适配器
 */
public class TakeOrderAdapter extends RecyclerView.Adapter<TakeOrderAdapter.myViewHodler> {
    private Context context;
    private List<RestingInfoBean> rechargeBeans;
    private int currentIndex = 0;

    //创建构造函数
    public TakeOrderAdapter(Context context, List<RestingInfoBean> goodsEntityList) {
        //将传递过来的数据，赋值给本地变量
    ...
    public myViewHodler onCreateViewHolder(ViewGroup parent, int viewType) {
        //创建自定义布局
        View itemView = View.inflate(context, R.layout.item_take_order_layout, null);
    ...
    public void onBindViewHolder(final myViewHodler holder, @SuppressLint("RecyclerView") final int position) {
        ...
        //根据点击位置绑定数据
        RestingInfoBean data = rechargeBeans.get(position);

        holder.bindData(rechargeBeans.get(position), position, currentIndex);

        holder.order_id.setText("取单号："+data.getOrderNumberBean().getOrderNumberStr());//获取实体类中的name字段并设置
        holder.order_time.setText("订单时间："+data.getOrderNumberBean().getCreatTimeStr());//订单时间
        holder.order_prick.setText(data.getOrderNumberBean().getOrder_price());
    ...
    class myViewHodler extends RecyclerView.ViewHolder {
        ...
        public void bindData(RestingInfoBean saleBillBean, int position, int currentIndex) {
...
//            mItemGoodsName.setText(();
    ...
    public interface OnClickListener {
        void onClick(RestingInfoBean lists, int position);
    }
    ...
    public interface OnItemClickListener {
         ...
         */
        public void OnItemClick(View view, RestingInfoBean data, int position);
    }
