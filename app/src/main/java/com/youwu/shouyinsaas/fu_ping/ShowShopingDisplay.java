 ...
 */
package com.youwu.shouyinsaas.fu_ping;
...
import com.youwu.shouyinsaas.fu_ping.adapter.RestingGoodsAdapter;
import com.youwu.shouyinsaas.sunmi.YWStringUtils;
...
import com.youwu.shouyinsaas.utils_view.CircleImageView;
...
//      YWGoodBean ywGoodBean = BeanCloneUtil.cloneTo(ywGood);
        ...
        for (int i = 0; i < shopCarYWGoodBeans.size(); i++) {
            if (ywGoodBean instanceof CommunityBean && shopCarYWGoodBeans.get(i) instanceof CommunityBean && TextUtils.equals(((CommunityBean) shopCarYWGoodBeans.get(i)).getGoods_sku(), ((CommunityBean) ywGoodBean).getGoods_sku())) {
                if (from == 2) {
                    //如果是修改直接中断循环
                    hasInShopCar = i;
                    break;
                    ...
                    //不是修改判断折扣是否相等
//                    if (TextUtils.equals(((CommunityBean) shopCarYWGoodBeans.get(i)).getDiscount(), ((CommunityBean) ywGoodBean).getDiscount())) {
//                        hasInShopCar = i;
//                    } else {
//                        hasInShopCar = -1;
//                    }
...
//            shopCarYWGoodBeans.get(hasInShopCar).getSpecsBean().setBuynum(ywGoodBean.getSpecsBean().getBuynum());
            ...
            ((CommunityBean) shopCarYWGoodBeans.get(hasInShopCar)).setGoods_number(((CommunityBean) ywGoodBean).getGoods_number());
//            ((CommunityBean) shopCarYWGoodBeans.get(hasInShopCar)).setDiscount(((CommunityBean) ywGoodBean).getDiscount());
        }
    ...
    public void removeGoods(CommunityBean ywGoodBean) {

        int hasInShopCar = -1;
        for (int i = 0; i < shopCarYWGoodBeans.size(); i++) {
            if (shopCarYWGoodBeans.get(i) instanceof CommunityBean) {
                if (TextUtils.equals(((CommunityBean) shopCarYWGoodBeans.get(i)).getGoods_sku(), ((CommunityBean) ywGoodBean).getGoods_sku())) {
                    hasInShopCar = i;
                }
            }
        }
        ...
        int number = ywGoodBean.getGoods_number();
        if (number == 0) {
            shopCarYWGoodBeans.remove(hasInShopCar);
        } else {
            ((CommunityBean) shopCarYWGoodBeans.get(hasInShopCar)).setGoods_number(number);
        }
    ...
    public void setCouponName(CommunityBean ywGoodBean) {
        tvDuihuan.setVisibility(View.VISIBLE);
        tvDuihuan.setText("已兑换" + ywGoodBean.getGoods_name() + "1份");
    }
    ...
    public void setvip(VipBean vipBean) {
        Glide.with(outerContext)
                .load(vipBean.getHead_img())
                .apply(AppApplication.options)
                .into(vipHeadImage);
        vipMoney.setText("余额：" + vipBean.getBalance() + "元");
        vipName.setText(vipBean.getNick_name());
...
//        if (MyApplication.spUtils.getString("bannerType").equals("imageSbutton")){
//            //显示图片
//           if (MyApplication.systemBannerBean.getItems() == null){
//               List<Integer> bannerImgList = new ArrayList<>();
//               int resource = R.mipmap.bg;
//               bannerImgList.add(resource);
//               int time = Integer.parseInt( MyApplication.spUtils.getString("switchingTime") );
//               banner.setDelayTime(time * 1000);     //设置轮播时间
//               banner.setImages(bannerImgList)
//                       .setImageLoader(new GlideImageLoader())
//                       .start();
//           }else {
//               List<String> bannerImgList = new ArrayList<>();
//               for (int i=0;i< MyApplication.systemBannerBean.getItems().size();i++){
//                   bannerImgList.add(MyApplication.systemBannerBean.getItems().get(i).getImage()+"");
//               }
//               int time = Integer.parseInt( MyApplication.spUtils.getString("switchingTime") );
//               banner.setDelayTime(time * 1000);     //设置轮播时间
//               banner.setImages(bannerImgList)
...
//        }else {
//            //显示视频
////            String videoUrl = MyApplication.systemBannerBean.getVideo()+"";
